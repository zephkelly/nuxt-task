...
export abstract class ClientBaseStorage
  extends BaseStorage
  implements CronStorage {
  protected abstract storage: Storage

  async init(): Promise<void> {}

  async add(
    Task: Omit<CronTask, 'id' | 'createdAt' | 'updatedAt'>,
  ): Promise<CronTask> {
    const newTask = this.createTaskObject(Task)

          this.storage.setItem(this.getKey(newTask.id), JSON.stringify(newTask))
...
export async function createClientStorage(
  ...
  throw new Error(
    `Storage type ${options.type} is not supported in browser environment`,
        );
}
