...
describe('MemoryStorage', () => {
    ...
    describe('add', () => {
        ...
        it('should throw error when adding task with existing id', async () => {
            ...
            await expect(storage.add(taskData)).rejects.toThrow('Task with id duplicate-id already exists')
        })
    })
    ...
    describe('task status transitions', () => {
        it('should handle task status changes', async () => {
            ...
            // Update to running
            const runningTask = await storage.update(task.id, { status: 'running' })
            expect(runningTask.status).toBe('running')

            // Update to completed
            const completedTask = await storage.update(task.id, { status: 'completed' })
            expect(completedTask.status).toBe('completed')

            // Update back to pending
            const pendingTask = await storage.update(task.id, { status: 'pending' })
