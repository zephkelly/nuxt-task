...
} from '../../../src/runtime/utils/timezone'
...
describe('Scheduler', () => {
    ...
    const setupScheduler = async (moduleOpts: ModuleOptions) => {
        ...
        scheduler = new Scheduler(storage, moduleOpts, {
            tickInterval: 100,
            maxConcurrent: 2,
        })
    ...
    describe('timezone handling', () => {
        ...
        describe('task execution', () => {
            it('should handle task failures and retries', async () => {
                ...
                vi.spyOn(storage, 'update').mockImplementation(
                    async (id, updates) => ({
                        ...testTask,
                        ...updates,
                        id,
                ...
                expect(events.filter(e => e.type === 'retry')).toHaveLength(
                    1
                );
            ...
            const setupScheduler = async (
                ...
                scheduler = new Scheduler(storage, moduleOptions, {
                    tickInterval: 100,
                    maxConcurrent: 2,
                    ...baseOptions,
                })
        ...
        describe('DST transitions', () => {
            ...
            it('should maintain correct intervals across DST boundaries', async () => {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    intervals.push(
                        (currentTime.getTime() - lastRun.getTime())
                        / (60 * 60 * 1000),
                    );
                    lastRun = currentTime
                }
        ...
        describe('timezone validation', () => {
            it('should validate ambiguous times during DST transitions', async () => {
                ...
                expect(nextRun.getTime()).toBe(
                    new Date('2024-11-03T06:00:00.000Z').getTime(),
                );
            })
        })
    })
    ...
    describe('Scheduler Task Management', () => {
        ...
        describe('pauseTask and resumeTask', () => {
            ...
            it('should not resume a non-paused task', async () => {
                ...
                expect(unchangedTask?.metadata.nextRun).toEqual(
                    originalNextRun
                );
            })
        })
        ...
        describe('waitForTask', () => {
            ...
            it('should resolve immediately for non-existent tasks', async () => {
                const waitPromise = (scheduler as any).waitForTask(
                    "non-existent-id"
                );
